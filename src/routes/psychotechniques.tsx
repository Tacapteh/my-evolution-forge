...
function PsychoPage() {
  ...
  const psychoTask = mission.tasks.find((task) => task.type === "psycho");
        ...
        <Card className="card-forge p-5">
          ...
          {history.length === 0 ? (
            <div className="text-sm text-muted-foreground">
              Rien encore. Lance ton premier chrono.
            </div>
          ) : (
              ...
              {history.map(([date, d]) => (
                <li key={date} className="py-2.5 text-sm flex items-center gap-3">
                  ...
                  <div className="text-xs">
                    {d.psycho?.score != null && (
                      <span className="text-primary font-medium">{d.psycho.score}</span>
                    )}
                    {d.psycho?.duration != null && (
                      <span className="text-muted-foreground ml-2">
                        {format(d.psycho.duration)}
                      </span>
                    )}
                  </div>
                </li>
